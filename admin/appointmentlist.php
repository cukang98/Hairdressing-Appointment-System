								...
								if($a)
								{
									...
									if($data != 0)
									{
										...
										while($row = mysqli_fetch_assoc($a))
										{
											...
											if($status=="Confirmed" && $today >= $row['date_appointment'])
											...
											if($status=="Completed" )
												echo ' / <a style="cursor:pointer" data-toggle="modal" data-target="#receipt'.$row['appointment_id'].'"title="Get Receipt"><img src="receipt.png" width="18px" style="margin-top:1px;"></a>';
											  ...
											  '<div class="modal-body">'.
												...
												'<input type="hidden" class="form-control" name="aid" value="'.$aid.'" readonly>'.
												  ...
												  '<div class="form-group">'.
													...
													'<textarea type="text" class="form-control" name="remark" value="'.$row['remark'].'">'.$row['remark'].'</textarea>'.
												  '</div>'.
												  '<div class="form-group" >'.
...
?>
<?php 
	if(isset($_GET['aid']) && isset($_GET['status'])){
		$aid = $_GET['aid'];
		$status1 = $_GET['status'];
		...
		if($status1 == 'Confirmed')
		{
			...
			while($row = mysqli_fetch_assoc($getcustinfo))
			{
				...
				$template = file_get_contents("confirmappointmenttemplate.php");
				...
				$to = $custemail;
				$subject = "AerySalon - Appointment Confirmed";
				$message = $template;
		...
		$update = mysqli_query($conn,"UPDATE appointment SET status = '$status1',update_date = '$updatedate' WHERE appointment_id = '$aid'");
		if($update)
		{
			echo"<script>
			Swal.fire ({
